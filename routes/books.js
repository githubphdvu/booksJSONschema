...
router.post("/", async (req, res, next)=>{
    try {
        ...
        const book = await Book.create(req.body)//create book in database
        return res.status(201).json({book})//201 created
    }
    catch (err) {return next(err)}
})
router.put("/:isbn",async (req,res,next)=>{//only required:author,language,pages,title,year
    try {
        if ("isbn" in req.body) return next({status: 400,message: "Not allowed"})
        const validation = validate(req.body, bookSchemaUpdate);
        if (!validation.valid) return next({status: 400,errors: validation.errors.map(e => e.stack)})
